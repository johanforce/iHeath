...
class M008_OutputAnalyzer {
    private final Activity activity;

    private final M008_ChartDrawer chartDrawer;

    private M008_MeasureStore store;
    ...
    M008_OutputAnalyzer(Activity activity, TextureView graphTextureView, Handler mainHandler) {
        this.activity = activity;
        this.chartDrawer = new M008_ChartDrawer(graphTextureView);
        this.mainHandler = mainHandler;
    }

    private boolean detectValley() {
        final int valleyDetectionWindowSize = 13;
        CopyOnWriteArrayList<M008_Measurement<Integer>> subList = store.getLastStdValues(valleyDetectionWindowSize);
        if (subList.size() < valleyDetectionWindowSize) {
            ...
            for (M008_Measurement<Integer> measurement : subList) {
    ...
    void measurePulse(TextureView textureView, M008_CameraService cameraService) {

        // 20 times a second, get the amount of red on the picture.
        // detect local minimums, calculate pulse.

        store = new M008_MeasureStore();
        ...
        timer = new CountDownTimer(measurementLength, measurementInterval) {
            ...
            public void onTick(long millisUntilFinished) {
                ...
                Thread thread = new Thread(() -> {
                    ...
                    if (detectValley()) {
                        ...
                        sendMessage(M008_NhipTim.MESSAGE_UPDATE_REALTIME, currentValue);
                    }
            ...
            public void onFinish() {
                CopyOnWriteArrayList<M008_Measurement<Float>> stdValues = store.getStdValues();
                ...
                for (int stdValueIdx = 0; stdValueIdx < stdValues.size(); stdValueIdx++) {
                    ...
                    M008_Measurement<Float> value = stdValues.get(stdValueIdx);
                ...
                sendMessage(M008_NhipTim.MESSAGE_UPDATE_FINAL, returnValueSb.toString());
